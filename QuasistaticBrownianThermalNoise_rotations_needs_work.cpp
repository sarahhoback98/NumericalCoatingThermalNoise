...
std::map<std::string, int> material_from_string{
    {"AlGaAs", 0}, {"Iso_AlGaAs", 1}, {"Iso_Ta2O5", 2}, {"Iso_FusedSilica", 3}};
...
std::map<std::string, int> profile_from_string{
    {"TEM00", 0}, {"TEM02", 1}, {"TEM20", 2}, {"TEM02minus20", 3}};
...
struct CylTransFunc {
  ...
  Point<3> operator()(const Point<3>& in) const {
    return Point<3>(in(2), in(1), -1. * in(0) - (halflength - d));
...
CylTransFunc::CylTransFunc(double coatThick, double halfCylThick)
    : d(coatThick), halflength(halfCylThick) {}
...
class ElasticProblem {
 public:
  ...
  ~ElasticProblem();
  std::vector<std::map<std::string, double>> get_results() {
    return results;
  };
  void run();
 ...
 private:
  ...
  // Young tensors Yijkl
  Tensor<4, dim> Y_Iso_FusedSilica, Y_AlGaAs, Y_Iso_Ta2O5, Y_Iso_AlGaAs,
      Y_Iso_Ta2O5_Phi, Y_Iso_FusedSilica_Phi, Y_AlGaAs_Phi, Y_Iso_AlGaAs_Phi;

  // The built-in Young's tensors have lame_mu and lame_lambda defined
  // These are the Lame parameters used to construct Y
  double lame_mu_FusedSilica, lame_lambda_FusedSilica, lame_mu_Ta2O5,
      lame_lambda_Ta2O5, lame_mu_AlGaAs, lame_lambda_AlGaAs;
  ...
  double lossPhi_Iso_FusedSilica, lossPhi_Iso_Ta2O5, lossPhi_AlGaAs,
      lossPhi_Iso_AlGaAs;

  // Loss Angles for Young's tensors that can be varied
  double lossPhi_AlGaAs_11, lossPhi_AlGaAs_12, lossPhi_AlGaAs_44,
      lossPhi_Ta2O5_bulk, lossPhi_Ta2O5_shear, lossPhi_Iso_FusedSilica_bulk,
      lossPhi_Iso_FusedSilica_shear, lossPhi_Iso_AlGaAs_bulk,
  ...
  double getRotatedYijkl(int whichYijkl, unsigned int ib, unsigned int jb,
                         unsigned int kb, unsigned int lb);
};
...
NeumannValues<dim>::NeumannValues(const double& inR0, const double& inF0,
                                  const int& in_beam_profile)
    : Function<dim>(dim), r0(inR0), F0(inF0), beam_profile(in_beam_profile) {}

// arXiv:1707.07774 Eq. (33) third integrand, Eq. (27), etc. Tzz = -F p(r),
// Txz=Tyz=0
template <int dim>
inline void NeumannValues<dim>::vector_value(const Point<dim>& p,
                                             Vector<double>& values) const {
  ...
  switch (beam_profile) {
    ...
    case(TEM02) :
      myValue = -1.0 * F0 * exp(myValue / (r0 * r0)) *
                (4.0 * p(1) * p(1) / (r0 * r0) - 2.0) *
                (4.0 * p(1) * p(1) / (r0 * r0) - 2.0);
      myValue /= 8.0 * M_PI * r0 * r0;
      break;
    case(TEM20) :
      myValue = -1.0 * F0 * exp(myValue / (r0 * r0)) *
                (4.0 * p(0) * p(0) / (r0 * r0) - 2.0) *
                (4.0 * p(0) * p(0) / (r0 * r0) - 2.0);
      myValue /= 8.0 * M_PI * r0 * r0;
      break;
    case(TEM02minus20) :
...
void NeumannValues<dim>::vector_value_list(
  ...
  for (unsigned int p = 0; p < n_points; ++p)
    NeumannValues<dim>::vector_value(points[p], value_list[p]);
}
...
//****************************************************************//
template <int dim>
double ElasticProblem<dim>::getYijkl(int whichYijkl, unsigned int i,
...
template <int dim>
double ElasticProblem<dim>::getRotatedYijkl(int whichYijkl, unsigned int ib,
                                            unsigned int jb, unsigned int kb,
                                            unsigned int lb) {
  double theta1, theta2, theta3;
  theta1 = M_PI / 15;  // first rotation (yaw)
  theta2 = M_PI / 15;  // second rotation (pitch)
  theta3 = M_PI / 15;  // third rotation (roll)
  ...
  for (size_t ib = 0; ib < dim; ++ib) {
    for (size_t jb = 0; jb < dim; ++jb) {
      for (size_t kb = 0; kb < dim; ++kb) {
        for (size_t lb = 0; lb < dim; ++lb) {
          Yijkl_bar[ib][jb][kb][lb] = 0.0;
        }
      }
    }
  }
...
ElasticProblem<dim>::ElasticProblem()
  ...
  // kIso_Ta2O5 #tantalum
  // kIso_FusedSilica #fused silica
  mWhichCoatingYijkl = material_from_string.at(
      config["Mirror"]["Materials"]["CoatingMaterial"].as<std::string>());
  mWhichSubstrateYijkl = material_from_string.at(
      config["Mirror"]["Materials"]["SubstrateMaterial"].as<std::string>());
  ...
  F0 = config["LaserBeam"]["F0"]
           .as<double>();  // final result should not depend on this

  // Choose the size of the mirror: it's a rectangle with points
  // low = (-innerMirrorSize*outFac, -innerMirrorSize*outFac,
  // -innerMirrorSize*outFac) up  = (innerMirrorSize*outFac,
  // innerMirrorSize*outFac, 0) Later, I will insert a coating above z=0,
  // and move the outer boundary.
  rad = config["Mirror"]["Dimensions"]["Radius"].as<double>();  // 25800.;
  innerMirrorSize = 4. * r0;       // this isn't the full size, but is used to
                                   // help concentrate points near the spacer
  outFac = rad / innerMirrorSize;  // so total mirror size is rad
                                   // diameter of substrate is 25000 micron
                                   // in Cole+ (2013).

  d = config["Mirror"]["Dimensions"]["CoatingThickness"]
          .as<double>();  // coating extends above z=0 this far...
                          // 6.83 micron: Cole+ 2013
                          // 4.68 based on Table I of Chalermsongsak+ (2015)
                          // earlier versions of this code used 4.68

  substrateheight =
      config["Mirror"]["Dimensions"]["SubstrateHeight"].as<double>();
  ...
  lossPhi_AlGaAs_12 =
      config_multloss["Crystalline_AlGaAs"]["Phi12"].as<double>();
  lossPhi_AlGaAs_44 =
      config_multloss["Crystalline_AlGaAs"]["Phi44"].as<double>();

  lossPhi_Iso_FusedSilica_bulk =
      config_multloss["Fused_Silica"]["Bulk"].as<double>();
  lossPhi_Iso_FusedSilica_shear =
      config_multloss["Fused_Silica"]["Shear"].as<double>();

  lossPhi_Ta2O5_bulk = config_multloss["Ta2O5"]["Bulk"].as<double>();
  lossPhi_Ta2O5_shear = config_multloss["Ta2O5"]["Shear"].as<double>();
  ...
  halflength =
      substrateheight / 2. + d / 2.;  // 1.0 in + coating = total thickness
  ...
  // Y = 9 mu K / (3K + mu), sigma = (3K - 2 mu)/(2(3k+mu)),
  // K=lambda+(2/3)mu
  ...
  Y_AlGaAs[0][0][0][0] = c11_AlGaAs;
  Y_AlGaAs[1][1][1][1] = c11_AlGaAs;
  Y_AlGaAs[2][2][2][2] = c11_AlGaAs;

  Y_AlGaAs[0][0][1][1] = c12_AlGaAs;
  Y_AlGaAs[0][0][2][2] = c12_AlGaAs;
  Y_AlGaAs[1][1][0][0] = c12_AlGaAs;
  Y_AlGaAs[1][1][2][2] = c12_AlGaAs;
  Y_AlGaAs[2][2][0][0] = c12_AlGaAs;
  Y_AlGaAs[2][2][1][1] = c12_AlGaAs;

  Y_AlGaAs[0][1][0][1] = c44_AlGaAs;
  Y_AlGaAs[0][1][1][0] = c44_AlGaAs;
  Y_AlGaAs[1][0][0][1] = c44_AlGaAs;
  Y_AlGaAs[1][0][1][0] = c44_AlGaAs;

  Y_AlGaAs[0][2][0][2] = c44_AlGaAs;
  Y_AlGaAs[0][2][2][0] = c44_AlGaAs;
  Y_AlGaAs[2][0][0][2] = c44_AlGaAs;
  Y_AlGaAs[2][0][2][0] = c44_AlGaAs;

  Y_AlGaAs[1][2][1][2] = c44_AlGaAs;
  Y_AlGaAs[1][2][2][1] = c44_AlGaAs;
  Y_AlGaAs[2][1][1][2] = c44_AlGaAs;
  Y_AlGaAs[2][1][2][1] = c44_AlGaAs;

  Y_AlGaAs_Phi[0][0][0][0] = c11_AlGaAs * lossPhi_AlGaAs_11;
  Y_AlGaAs_Phi[1][1][1][1] = c11_AlGaAs * lossPhi_AlGaAs_11;
  Y_AlGaAs_Phi[2][2][2][2] = c11_AlGaAs * lossPhi_AlGaAs_11;

  Y_AlGaAs_Phi[0][0][1][1] = c12_AlGaAs * lossPhi_AlGaAs_12;
  Y_AlGaAs_Phi[0][0][2][2] = c12_AlGaAs * lossPhi_AlGaAs_12;
  Y_AlGaAs_Phi[1][1][0][0] = c12_AlGaAs * lossPhi_AlGaAs_12;
  Y_AlGaAs_Phi[1][1][2][2] = c12_AlGaAs * lossPhi_AlGaAs_12;
  Y_AlGaAs_Phi[2][2][0][0] = c12_AlGaAs * lossPhi_AlGaAs_12;
  Y_AlGaAs_Phi[2][2][1][1] = c12_AlGaAs * lossPhi_AlGaAs_12;

  Y_AlGaAs_Phi[0][1][0][1] = c44_AlGaAs * lossPhi_AlGaAs_44;
  Y_AlGaAs_Phi[0][1][1][0] = c44_AlGaAs * lossPhi_AlGaAs_44;
  Y_AlGaAs_Phi[1][0][0][1] = c44_AlGaAs * lossPhi_AlGaAs_44;
  Y_AlGaAs_Phi[1][0][1][0] = c44_AlGaAs * lossPhi_AlGaAs_44;

  Y_AlGaAs_Phi[0][2][0][2] = c44_AlGaAs * lossPhi_AlGaAs_44;
  Y_AlGaAs_Phi[0][2][2][0] = c44_AlGaAs * lossPhi_AlGaAs_44;
  Y_AlGaAs_Phi[2][0][0][2] = c44_AlGaAs * lossPhi_AlGaAs_44;
  Y_AlGaAs_Phi[2][0][2][0] = c44_AlGaAs * lossPhi_AlGaAs_44;

  Y_AlGaAs_Phi[1][2][1][2] = c44_AlGaAs * lossPhi_AlGaAs_44;
...
void ElasticProblem<dim>::setup_system() {
  ...
  // if you don't yet know the final sparsity pattern,
  // DynamicSparsityPattern works better than
  // CompressedSimpleSparsityPattern csp (locally_relevant_dofs);
  DynamicSparsityPattern csp(locally_relevant_dofs);

  DoFTools::make_sparsity_pattern(dof_handler, csp, constraints, false);
  SparsityTools::distribute_sparsity_pattern(
      csp, dof_handler.n_locally_owned_dofs_per_processor(), mpi_communicator,
      locally_relevant_dofs);  // ensure each proc knows all relevant dofs

  system_matrix.reinit(locally_owned_dofs, locally_owned_dofs, csp,
                       mpi_communicator);
}
...
void ElasticProblem<dim>::assemble_system() {
  ...
  for (; cell != endc; ++cell) {
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int A = 0; A < dofs_per_cell; ++A) {
        const unsigned int component_A = fe.system_to_component_index(A).first;
        for (unsigned int B = 0; B < dofs_per_cell; ++B) {
          const unsigned int component_B =
              fe.system_to_component_index(B).first;
            ...
            for (unsigned int k = 0; k < dim; ++k) {
              ...
              for (unsigned int q_point = 0; q_point < n_q_points; q_point++) {
                ...
                if (fe_values.quadrature_point(q_point)(dim - 1) > 0.) {
                  // Yijkl = getYijkl(mWhichCoatingYijkl, i, component_A, k,
                  // component_B);
                  double theta1, theta2, theta3;
                  theta1 = 0.0;  // first rotation (yaw)
                  theta2 = 0.0;  // second rotation (pitch)
                  theta3 = 0.0;  // third rotation (roll)

                  std::array<std::array<double, 3>, 3> rotation_3D{
                      {{cos(theta1) * cos(theta2), cos(theta2) * sin(theta1),
                        -sin(theta2)},
                       {cos(theta1) * sin(theta2) * sin(theta3) -
                            cos(theta3) * sin(theta1),
                        cos(theta1) * cos(theta3) +
                       ...
                       {cos(theta1) * cos(theta3) * sin(theta2) +
                        ...
                        cos(theta2) * cos(theta3)}}};
                  std::array<
                      std::array<std::array<std::array<double, dim>, dim>, dim>,
                      dim> Yijkl_bar;
                              ...
                              for (size_t l = 0; l < dim; ++l) {
                                for (size_t lb = 0; lb < dim; ++lb) {
                                  Yijkl_bar[ib][jb][kb][lb] =
                                      rotation_3D[ib][i] * rotation_3D[jb][j] *
                                      rotation_3D[kb][k] * rotation_3D[lb][l] *
                                      getYijkl(mWhichCoatingYijkl, i,
                                               component_A, k, component_B);
                                }
                              }
                            }
                          }
      ...
      for (unsigned int face_number = 0;
        ...
        if (cell->face(face_number)->at_boundary() &&
          ...
          for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point) {
            ...
            neumann.vector_value(fe_face_values.quadrature_point(q_point),
                                 neumann_value);
  ...
  // boundary conditions on boundaries flagged as zero (all but the z=0
  // face). ZeroFunction<dim>(dim) sets all dim=3 vector components: each
  // becomes a dim=3 scalar function that's zero everywhere.
...
unsigned int ElasticProblem<dim>::solve() {
  ...
  SolverControl solver_control(dof_handler.n_dofs(), 1.e-12);
...
void ElasticProblem<dim>::refine_grid() {
  ...
  parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(
      triangulation, local_error_per_cell, 0.14, 0.02);
...
void ElasticProblem<dim>::output_results(const unsigned int cycle) {
  ...
  for (; cell != endc; ++cell) {
    if (cell->is_locally_owned()) {
      ...
      // at quadrature point q. So u_{j;i}(x_q) =
      // solution_gradients[q][j][i]
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point) {
        for (int i = 0; i < dim; ++i) {
          for (int j = 0; j < dim; ++j) {
            for (int k = 0; k < dim; ++k) {
              for (int l = 0; l < dim; ++l) {
                local_U_this_energy = 0.5 * fe_values.JxW(q_point) *
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point) {
        for (int i = 0; i < dim; ++i) {
          for (int j = 0; j < dim; ++j) {
            for (int k = 0; k < dim; ++k) {
              for (int l = 0; l < dim; ++l) {
                local_thisEnergy = 0.5 * fe_values.JxW(q_point) *
                                   solution_gradients[q_point][j][i] *
                                   solution_gradients[q_point][l][k];
  ...
  // Note: I used 1=1TPa=1e12 Pa before, so I'm a factor of 1e12 away from
  // SI units
  ...
  pcout << "Total amp noise (m/sqrt(Hz)) * sqrt(f/Hz): "
        << totalAmpNoiseTimesSqrtf << std::endl;
  pcout << "Substrate amp noise (m/sqrt(Hz)) * sqrt(f/Hz): "
        << subAmpNoiseTimesSqrtf << std::endl;
  pcout << "Coating amp noise (m/sqrt(Hz)) * sqrt(f/Hz): "
        << coatAmpNoiseTimesSqrtf << std::endl;
    ...
    if (cycle == 0)  // set up the grid
    {
      ...
      for (unsigned int j = 0; j < refineByDIts; ++j) {
        ...
        for (; cell != endc; ++cell) {
          for (unsigned int v = 0; v < GeometryInfo<dim>::vertices_per_cell;
            ...
            if (fabs(vertexDist) < r0 && vertexRad < (refineByDIts - j) * r0) {
        ...
        triangulation.execute_coarsening_and_refinement();
        pcout << "done. Refinement level: " << triangulation.n_global_levels()
              << "." << std::endl;
      }
      ...
      for (; cell != endc; ++cell)
        for (unsigned int face_number = 0;
          ...
          if (d - cell->face(face_number)->center()(dim - 1) < 1.e-12) {
...
int main(int argc, char** argv) {
  // There is one command-line option we need: a path to the configuration
  // files
  ...
  // Catch::Session session_multloss;
  auto cli =
      session.cli() |
  ...
  auto cli_multloss =
      session.cli() |
      Catch::clara::Opt(
          config_multloss_file,
          "config_multloss_file")["-m"]["--configuration_multloss"](
...
TEST_CASE("TestEnergies") {
  ...
  const double expected_energy =
      config["Tests"]["ExpectedTotalEnergy"].as<double>();
  const double expected_coating_energy =
      config["Tests"]["ExpectedCoatingEnergy"].as<double>();
  const double expected_noise =
      config["Tests"]["ExpectedTotalNoiseAt1Hz"].as<double>();
  const double expected_coating_noise =
      config["Tests"]["ExpectedCoatingNoiseAt1Hz"].as<double>();

  const double eps = config["Tests"]["Eps"].as<double>();
  const int cycle_to_test = config["Tests"]["CycleToTest"].as<int>();

  const double energy =
      elastic_problem_3d.get_results()[cycle_to_test].at("energy");
  const double coating_energy =
      elastic_problem_3d.get_results()[cycle_to_test].at("coatingEnergy");
  const double noise = elastic_problem_3d.get_results()[cycle_to_test]
                           .at("totalAmpNoiseTimesSqrtf");
  const double coating_noise = elastic_problem_3d.get_results()[cycle_to_test]
                                   .at("coatAmpNoiseTimesSqrtf");

  REQUIRE(fabs(energy - expected_energy) / expected_energy < eps);
  REQUIRE(fabs(coating_energy - expected_coating_energy) /
              expected_coating_energy <
          eps);
  REQUIRE(fabs(noise - expected_noise) / expected_noise < eps);
  REQUIRE(fabs(coating_noise - expected_coating_noise) /
              expected_coating_noise <
          eps);
}
