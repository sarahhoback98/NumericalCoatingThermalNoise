...
ElasticProblem<dim>::ElasticProblem()
    : mpi_communicator(MPI_COMM_WORLD),  // MPI_COMM_WORLD=all procs available
      ...
      fe(FE_Q<dim>(2),
         dim),  // use 3D quad elements, and use dim=3
...
TEST_CASE("TestEnergies") {
  ...
  const double noise = elastic_problem_3d.get_results()[cycle_to_test].at(
      "totalAmpNoiseTimesSqrtf");
  const double coating_noise =
      elastic_problem_3d.get_results()[cycle_to_test].at(
